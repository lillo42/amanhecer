...
/// <summary>
/// Declares <see cref="AmanhecerTelemetryMiddleware"/> on the annotated handler class or
...
/// </summary>
public class AmanhecerTelemetryMiddleware : IMiddleware
{
    ...
    private static readonly Counter<int> TimeoutCounter = AmanhecerDiagnostics.Meter.CreateCounter<int>(
        "amanhecer.request.processing.timeout",
        unit: "{request}",
    ...
    private static readonly Counter<int> CancelledCounter = AmanhecerDiagnostics.Meter.CreateCounter<int>(
        "amanhecer.request.processing.cancelled",
        unit: "{request}",
    ...
    public async ValueTask ExecuteAsync(IPipelineContext context, Func<IPipelineContext, ValueTask> next)
    {
        ...
        var activity = AmanhecerDiagnostics.ActivitySource.StartActivity($"{context.RoutingKey} process",
        ...
        try
        {
            ...
            {
                metadata.Add(new KeyValuePair<string, object?>("amanhecer.response.type",

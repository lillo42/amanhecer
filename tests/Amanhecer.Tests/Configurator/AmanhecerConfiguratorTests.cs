...
public class AmanhecerConfiguratorTests
{
    private const string DynamicAssemblyName = "Amanhecer.Tests.DynamicDoubles";
    ...
    public async Task When_AutoFromAssemblies_Should_RegisterMiddlewares()
    {
        ...
        var configurator = new AmanhecerConfigurator(services);
    ...
    public async Task When_AutoFromAssemblies_WithoutAssemblies_Should_ScanCallingAssembly()
    {
        ...
        await Assert.That(() => configurator.AutoFromAssemblies())
            .Throws<Exception>()
            .WithMessageContaining("Amanhecer.Tests");
    }

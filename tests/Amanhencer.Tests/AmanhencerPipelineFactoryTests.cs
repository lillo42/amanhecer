...
public class AmanhencerPipelineFactoryTests
{
    ...
    {
        var middlewareType = typeof(AmanhencerPipelineFactoryTests);
        object? metadata = new { Key = "value" };
        var options = new Dictionary<string, ImmutableList<ImmutableList<AmanhencerMiddlewareOptions>>>
            {
                ["key"] =
                [
                    [new AmanhencerMiddlewareOptions(middlewareType, 0, metadata)]
                ]
            }
            .ToFrozenDictionary();
        ...
        var options = new Dictionary<string, ImmutableList<ImmutableList<AmanhencerMiddlewareOptions>>>
            {
                ["key"] =
                [
                    [new AmanhencerMiddlewareOptions(typeof(string), 0, null)],
                    [new AmanhencerMiddlewareOptions(typeof(int), 1, "meta")]
                ]
            }
            .ToFrozenDictionary();
        ...
        var options = new Dictionary<string, ImmutableList<ImmutableList<AmanhencerMiddlewareOptions>>>
            {
                ["key"] =
                [
                    ImmutableList<AmanhencerMiddlewareOptions>.Empty
                ]
            }
            .ToFrozenDictionary();
